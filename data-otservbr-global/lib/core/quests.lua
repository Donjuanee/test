if not Quests then
	Quests = {
		...
		[51] = {
			name = "Beginner's Trials",
			startStorageId = Storage.Custom.BeginnersTrials.Questline,
			startStorageValue = 1,
			missions = {
				[1] = {
					name = "Rotworm Trouble",
					-- IMPORTANT (client refresh): this mission uses the kill-counter storage
					-- as its storageId. That way, every kill changes the mission storage value,
					-- which forces the Tibia client to refresh Quest Log + enables Quest Tracker.
					storageId = Storage.Custom.BeginnersTrials.Kills,
					missionId = 90001,
					startValue = 0,
					endValue = 101,
					description = function(player)
						...
						if kills < 100 then
							return string.format(
								"Eldric Wayfarer asked you to prove yourself by slaying 100 cave-dwellers near the rotworm tunnels. Progress: %d/100.",
								kills
							)
						elseif kills == 100 then
							return "You have slain enough creatures. Return to Eldric Wayfarer for your reward."
						else
							return "You helped thin out the tunnels and earned Eldric's trust. Your journey has just begun!"
						end
					end,
				},
				[2] = {
					name = "Legion Helmet",
					storageId = Storage.Custom.BeginnersTrials.LegionHelmet,
					missionId = 90002,
					startValue = 1,
					endValue = 3,
					states = {
						[1] = "Eldric Wayfarer asked you to bring him a legion helmet.",
						[2] = "You have the legion helmet. Return to Eldric Wayfarer for your reward.",
						[3] = "You brought Eldric a legion helmet and received your reward.",
					},
				},
			},
		},
